...
import {
  ...
  SKUS, CANAIS, CATEGORIAS, TIPOS_ESTOQUE, CONFIG, SKU_COLORS
} from "@/lib/store";
...
const EstoquePage = () => {
  ...
  const chartData = SKUS.map(sku => ({ name: sku, value: saldoPorSku[sku] || 0 }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await updateEstoque(editingId, { data: form.data, tipo: form.tipo as "Entrada" | "Saída", categoria: form.categoria, canal: form.canal, sku: form.sku, quantidade: qty });
        setRecords(prev => prev.map(r => r.id === editingId ? { ...r, data: form.data, tipo: form.tipo as "Entrada" | "Saída", categoria: form.categoria, canal: form.canal, sku: form.sku, quantidade: qty } : r));
        toast.success("Movimentação atualizada!");
      } else {
        const newRecord = await insertEstoque({ data: form.data, tipo: form.tipo as "Entrada" | "Saída", categoria: form.categoria, canal: form.canal, sku: form.sku, quantidade: qty });
  ...
  const handleEdit = (r: EstoqueRecord) => {
    setEditingId(r.id);
    setForm({ data: r.data, tipo: r.tipo, categoria: r.categoria, canal: r.canal, sku: r.sku, quantidade: String(r.quantidade) });
    setOpen(true);
  };
      ...
      <div className="mx-auto max-w-7xl px-6 py-8 space-y-8 animate-fade-in">
        ...
        <div className="rounded-lg border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border p-4">
            ...
            <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditingId(null); setForm({ data: "", tipo: "", categoria: "", canal: "", sku: "", quantidade: "" }); } }}>
          ...
          <div className="flex flex-wrap gap-3 border-b border-border p-4 bg-muted/30">
            ...
            <Select value={filterSku} onValueChange={setFilterSku}>
              <SelectTrigger className="w-[140px] h-8 text-xs"><SelectValue placeholder="SKU" /></SelectTrigger>
